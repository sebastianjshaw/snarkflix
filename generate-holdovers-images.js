...
/**
 * Generate responsive images for The Holdovers (2023) review
 * 
 * This script generates responsive image sizes for the The Holdovers review header image.
 * It uses the existing PNG file as the source.
 * 
 * Usage:
 *   Run: node generate-holdovers-images.js
 */
...
const REVIEW_DIR = path.join(__dirname, 'images/reviews/the-holdovers-2023');
const SOURCE_IMAGE = path.join(REVIEW_DIR, 'header-the-holdovers-2023.png');
const BASE_NAME = path.join(REVIEW_DIR, 'header-the-holdovers-2023');
...
function generateResponsiveImages() {
    if (!fs.existsSync(SOURCE_IMAGE)) {
        ...
        console.error('   Please ensure the source image exists at: images/reviews/the-holdovers-2023/header-the-holdovers-2023.png');
        process.exit(1);
    }

    console.log('🚀 Generating responsive images for The Holdovers (2023)...\n');
